 ...
 */
export function HSLColor (h: number, s: number, l: number): RGBColor {
	let r, g, b
 ...
 */
export function HSVColor (h: number, s: number, v: number): RGBColor {
...
export function randomHColor (): RGBColor {
	return HSVColor(Math.floor(Math.random() * 359), Math.random() * 0.2 + 0.8, Math.random() * 0.5 + 0.5)
}
